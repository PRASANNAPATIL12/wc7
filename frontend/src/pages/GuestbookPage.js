...
const GuestbookPage = ({ isPrivate = false, isDashboard = false }) => {
  ...
  // Get wedding ID for API calls - Always use weddingData.id
  // For all pages (public landing, private wedding page, dashboard), we need the actual wedding_id
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      // If no wedding ID, show demo messages for public landing page
      if (!weddingId) {
      ...
      // Get messages for this specific wedding
      let response;
      if (isPrivate && isDashboard) {
        ...
        // Public wedding page or dashboard - get messages for specific wedding
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate wedding ID exists
    if (!weddingId && !isDashboard) {
      setError('Wedding ID not found. Please refresh the page.');
      setSubmitting(false);
      return;
    }
    ...
    try {
      ...
      if (isDashboard && sessionId) {
        ...
        // Public guestbook or shareable wedding page - use actual wedding_id
        requestBody = {
          ...newMessage,
          wedding_id: weddingId
        };
        console.log('Submitting guestbook message:', requestBody);
