...
const ThemeManager = ({ currentTheme, setCurrentTheme, themes, onSave, theme }) => {
  ...
  const handleSaveTheme = async () => {
    ...
    try {
      console.log('💾 ThemeManager: Saving theme to backend...', selectedTheme);
      
      // Call the parent's onSave function which will update the full wedding document
      // This is the same pattern used by FAQ, OurStory, Gallery, and other sections
      await onSave({ theme: selectedTheme });
      
      console.log('✅ ThemeManager: Theme saved successfully!');
      alert('Theme saved successfully! It will persist across login/logout.');
    } catch (error) {
