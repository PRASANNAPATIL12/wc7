...
// Inner App component that has access to UserDataContext
function AppContent() {
  const { weddingData, isAuthenticated } = useUserData();
  ...
  // Load theme from weddingData when it changes (for authenticated users)
  useEffect(() => {
    if (isAuthenticated && weddingData && weddingData.theme) {
      console.log('Loading theme from weddingData:', weddingData.theme);
      setCurrentTheme(weddingData.theme);
      // Also save to localStorage for persistence
      localStorage.setItem('selectedTheme', weddingData.theme);
    }
  }, [weddingData, isAuthenticated]);
